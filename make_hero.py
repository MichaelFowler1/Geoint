...
headline = f"{n_planes} aircraft detected" + (
    f"  +{n_other} vehicles" if n_other else ""
)
fig.text(0.95, 0.960, headline, ha="right", fontsize=14, fontweight="bold", color=ACC)
fig.text(
    0.05,
    0.922,
    "Overhead imagery  →  GPU detector (RT-DETR / YOLO-OBB)  →  "
...
for poly, conf in zip(polys, confs):
    ax.add_patch(Polygon(poly, closed=True, fill=False, edgecolor=ACC, linewidth=1.6))
